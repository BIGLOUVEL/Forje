...
async function runAgent1(url) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    track({ feature: 'onboarding', model: 'claude-haiku-4-5-20251001', inputTokens: response.usage?.input_tokens, outputTokens: response.usage?.output_tokens });
    const searches = response.content.filter(b => b.type === 'tool_use').length;
    for (let s = 0; s < searches; s++) track({ feature: 'onboarding_search', model: 'web_search' });
