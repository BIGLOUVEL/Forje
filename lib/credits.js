...
const CREDIT_COSTS = {
  ...
  // ─── Blaise (directeur artistique IA) ───
  // Logo GPT-Image-2 planche mark seul + déclinaisons — ~0,17€
  blaise_logo: 2,
  // Retouche Nano Banana (Gemini image) — ~0,05€
  blaise_edit: 1,
};
...
function getCreditCost(postType, variant = 'light') {
  ...
  if (postType === 'blaise_logo') return CREDIT_COSTS.blaise_logo;
  if (postType === 'blaise_edit') return CREDIT_COSTS.blaise_edit;
