...
const BLAISE_TOOLS = [
  ...
  },
  {
    ...
    description: "Cherche des images de référence (Serper) : photos de personnes, lieux, ambiances d'une niche, exemples de POSTS de médias. Gratuit. INTERDIT pour les logos, monogrammes, icônes ou tout design graphique — Google Images ne retourne que du stock générique qui contredit les standards de qualité. Pour l'inspiration logo : directions verbales + marques citées par leur nom.",
    input_schema: {
      type: 'object',
...
// Colonnes touchées par applyLogoVariants — journalisées ensemble pour que
// l'annulation restaure le logo complet (badge + nu + style).
const LOGO_COLUMNS = ['logo_url', 'logo_badge_url', 'logo_nu_url', 'logo_style'];

// Journal d'application (table identity_changes) : old/new de chaque champ,
// AVANT application → bouton "Annuler" (24h) sur la card système du fil.
async function logIdentityChange(clientId, changedFields) {
  if (!Object.keys(changedFields).length) return null;
  const { data, error } = await supabase.from('identity_changes')
    .insert({ client_id: clientId, changed_fields: changedFields, source: 'blaise' })
    .select('id').single();
  if (error) { console.warn('[Blaise/journal]', error.message); return null; }
  return data.id;
}
...
async function executeUpdateIdentity(input, client, clientId) {
  ...
  // État actuel AVANT application (lecture fraîche — le client passé en
  // paramètre peut être en retard d'un tour) → journal old/new.
  const { data: before } = await supabase.from('clients')
    .select([...IDENTITY_FIELDS, ...LOGO_COLUMNS].join(','))
    .eq('id', clientId).maybeSingle();
  const changedFields = {};
  for (const k of applied) changedFields[k] = { old: before ? before[k] : null, new: patch[k] };
  ...
  if (input.logo_url) {
    ...
    if (logoBuf) {
      ...
      try {
        ...
        // Les fichiers archivés ne sont jamais supprimés → les anciennes URLs
        // restent valides, l'annulation peut les restaurer telles quelles.
        const { data: after } = await supabase.from('clients')
          .select(LOGO_COLUMNS.join(',')).eq('id', clientId).maybeSingle();
        for (const k of LOGO_COLUMNS) {
          changedFields[k] = { old: before ? before[k] : null, new: after ? after[k] : null };
        }
      } catch (e) {
  ...
  const changeId = await logIdentityChange(clientId, changedFields);
  return { applied, change_id: changeId, changed_at: new Date().toISOString() };
}
...
async function executeSaveStrategy(input, clientId) {
  ...
  const { data: before } = await supabase.from('clients')
    .select('strategy').eq('id', clientId).maybeSingle();
  ...
  const changeId = await logIdentityChange(clientId, { strategy: { old: before ? before.strategy : null, new: strategy } });
  return { saved: true, strategy, change_id: changeId, changed_at: new Date().toISOString() };
}
...
// Garde-fou : Google Images sur une requête "logo" ne ramène QUE du stock
// générique (Shutterstock, "Slogan Here"…) — les anti-patterns exacts de la
// KB logos. Aucune requête ne corrige ça → refus net, le modèle rebondit en
// directions verbales (règle RÉFÉRENCES VISUELLES du system prompt).
const LOGO_QUERY_RE = /logo|logotype|monogram|lettermark|wordmark|brand\s*mark|ic[oô]nes?\b|icons?\b|embl[eè]me|pictogram/i;

async function executeSearchReferences(input) {
  if (LOGO_QUERY_RE.test(String(input.query || ''))) {
    return {
      error: 'refs_not_allowed_for_logos',
      hint: "décris la direction verbalement (mots visuels + marques citées par leur nom) — pas d'images pour le design graphique",
    };
  }
  try {
